<template>
  <div class="m-auto text-center grid gap-6 max-w-[30rem]" flat>
    ...
    <div class="grid gap-6 w-full">
      <FloatLabel>
        <InputText v-tooltip.top="'Enter your name'" v-model="email" filled class="w-full" :invalid="!isValidEmail(email)" label="Email" type="email" required />
        <label for="email">Email</label>
      </FloatLabel>
      <FloatLabel>
        <Password v-model="password" class="w-full" :input-style="{ width: '100%' }" toggleMask :invalid="!isValidPassword(password)" required />
